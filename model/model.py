...
class GruKRegion(nn.Module):
    ...
    def __init__(self, kernel_size=4, layers=4, out_size=8, dropout=0.5):
        super().__init__()
    ...
    def forward(self, raw):
        ...
        b, k, d = raw.shape # b=5 k=116 d=100
...
class Embed2GraphByProduct(nn.Module):
    ...
    def __init__(self, input_dim, roi_num=116):
        super().__init__()
...
class GNNPredictor(nn.Module):
    ...
    def __init__(self, node_input_dim, roi_num=116):
        super().__init__() # node_input_dim = roi_num=116
        inner_dim = roi_num
        ...
        self.layer_1 = nn.Linear(8*roi_num, 256)
        self.layer_2 = nn.Linear(256, 32)
        self.layer_3 = nn.Linear(32, 2)
        self.relu =  nn.LeakyReLU(negative_slope=0.2)

    def reduced2matrix(self, matrix, ratio):
        # Create an empty array to store the result
        matrix = matrix.cpu().detach().numpy()
        result = np.zeros_like(matrix)

        # Loop over the first dimension of the matrix
        for i in range(matrix.shape[0]):
            # Flatten the matrix
            flattened_matrix = matrix[i].flatten()
            threshold = np.percentile(flattened_matrix, ratio)
            result[i] = np.where(matrix[i] > threshold, matrix[i], 0)
...
class FBNETGEN(nn.Module):
    ...
    def __init__(self, model_config, roi_num=360, node_feature_dim=360, time_series=512):
        ...
        self.verify_reduced_model = model_config['reduced_matrix']
        self.verify_shrink_matrix = model_config['shrink_matrix']
        self.ratio = int(model_config['ratio'])
        if model_config['extractor_type'] == 'cnn':
            self.extract = ConvKRegion(
                out_size=model_config['embedding_size'], kernel_size=model_config['window_size'],
    ...
    def shrink2matrix(self, symmetric_matrix, nodes, ratio):
        symmetric_matrix = symmetric_matrix.cpu().detach().numpy()
        nodes = nodes.cpu().detach().numpy()

        # 创建一个空列表
        matrix_list = []
        nodes_list = []
        # Loop through each matrix in the symmetric_matrix
        for i in range(symmetric_matrix.shape[0]):
            # 计算每行的和
            row_sum = np.sum(symmetric_matrix[i], axis=1)

            # 排序行索引
            sorted_idx = np.argsort(row_sum)

            # 计算需要保留的行数
            num_rows = math.ceil(symmetric_matrix.shape[1] * ratio * 0.01)

            # 获取需要保留的行的索引
            keep_idx = sorted_idx[-num_rows:]
            # 对 keep_idx 数组进行排序
            keep_idx = np.sort(keep_idx)

            # 根据保留的行索引，将matrix进行缩减
            new_matrix = symmetric_matrix[i][keep_idx][:, keep_idx]
            new_nodes = nodes[i][keep_idx][:, keep_idx]

            # 将每个矩阵添加到列表中
            matrix_list.append(new_matrix)
            nodes_list.append(new_nodes)

        return np.array(matrix_list), np.array(nodes_list)
    ...
    def forward(self, t, nodes):
        x = self.extract(t) # x=[5,116,8] t=[5,116,100]
        x = F.softmax(x, dim=-1)
        m = self.emb2graph(x)

        m = m[:, :, :, 0] # m=nodes=[5,116,116]

        if self.verify_reduced_model:
            m = self.reduced2matrix(m, self.ratio)
            m = torch.from_numpy(m).float().to(device)

        if self.verify_shrink_matrix:
            m, nodes = self.shrink2matrix(m, nodes, self.ratio) # m=nodes= [5,58,58]
            m = torch.from_numpy(m).float().to(device)  # 数据格式由ndarray转为tensor
            nodes = torch.from_numpy(nodes).float().to(device)  # 数据格式由ndarray转为tensor

        bz, _, _ = m.shape

        edge_variance = torch.mean(torch.var(m.reshape((bz, -1)), dim=1)) # 将张量m视为5个大小为116*116的矩阵，对每个矩阵进行reshape操作，将其变为一个长度为116*116的向量。然后，对于每个向量，求出它的方差，最后对这5个方差值求平均数，最终返回这个平均值

        a,b, c = self.predictor(m, nodes)

        return a, b, edge_variance, c
