...
class Reader:
    ...
    def __init__(self, root_path, id_file_path=None) -> None:
        ...
        self.phenotype = os.path.join(root_folder, 'data/participants.tsv')
    ...
    def get_timeseries(self, subject_list, atlas_name, silence=False):
        ...
        for i in range(len(subject_list)):
            ...
            ro_file = [f for f in os.listdir(subject_folder) if f.endswith('_rois_' + atlas_name + '.txt')]
            for i in range(3):
                fl = os.path.join(subject_folder, ro_file[i])
                if silence != True:
                    print(ro_file[i])
                ...
                for i, subj_id in enumerate(subjects):
                    for j in range(3):
                        subject_file = os.path.join(save_path, subj_id,
                                                    subj_id + '_' + atlas_name + '_' + str(j) + kind.replace(' ', '_')+'.mat')
                        sio.savemat(subject_file, {'connectivity': connectivity[3*i+j]})
                return connectivity
            else:
                for i, subj_id in enumerate(subjects):
                    subject_file = os.path.join(save_path, subj_id,
                ...
                if row['participant_id'] in subject_list:
                    if score == 'HANDEDNESS_CATEGORY':
                        if (row[score].strip() == '-9999') or (row[score].strip() == ''):
                            scores_dict[row['participant_id']] = 'R'
                        elif row[score] == 'Mixed':
                            scores_dict[row['participant_id']] = 'Ambi'
                        elif row[score] == 'L->R':
                            scores_dict[row['participant_id']] = 'Ambi'
                        else:
                        ...
                        if (row[score].strip() == '-9999') or (row[score].strip() == ''):
                            scores_dict[row['participant_id']] = 100
                        else:
                            scores_dict[row['participant_id']] = float(row[score])
    ...
    def get_networks(self, subject_list, kind, iter_no='', seed=1234, n_subjects='', atlas_name="aal",
        ...
        for i, subject in enumerate(subject_list):
            ...
            fl = os.path.join(self.data_folder, subject,
                                subject + "_" + atlas_name + "_" + str(i%3) +kind.replace(' ', '_') + ".mat")
            print(fl)
